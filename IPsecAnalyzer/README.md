...
| 🔴 **CVE Threat Intel** | **Live NVD 2.0 API queries** (60-min cache, optional API key) for CVEs matching the detected cipher, with a small static catalog as an explicitly-labeled offline fallback; severity, exploit status and mitigation per CVE. | `com.ipsec.threat.NvdCveClient`, `ThreatIntelligenceEngine` |
...
│       │   │   ├── threat/NvdCveClient.java               # Live NVD 2.0 CVE feed (Layer 0)
│       │   │   ├── threat/ThreatIntelligenceEngine.java   # CVE matching + labeled offline fallback
...
| `ADMIN_PASSWORD` | *(generated)* | First-boot admin password; a random one is generated when unset |
| `ADMIN_CREDENTIALS_FILE` | `.admin-credentials` | Where the generated password is written once |
| `ADMIN_BOOTSTRAP_ENABLED` | `true` | Set `false` to disable first-boot admin creation |
| `MESH_SIGNING_KEY` | demo default (warns) | HMAC key for mesh gossip — **must be unique per deployment** |
| `NVD_API_KEY` | *(unset)* | Optional; raises NVD rate limits for the live CVE feed |
| `MORPHIC_ENABLED` | `true` | Scheduled 15-minute morphic config rotation |
| `MORPHIC_INTERVAL_MINUTES` | `15` | Rotation interval |
...
| `POST` | `/api/oracle/pre-harden` | **Real** autonomous pre-hardening: applies every generated plan, signs + gossips a mesh event, seals a ledger block (Bearer) |
...
TOKEN=$(curl -s -X POST http://127.0.0.1:8080/auth/login \
  ...
  -d '{"username":"admin","password":"<your-bootstrap-password>"}' | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
...
The seed user is inserted with `ON CONFLICT (username) DO NOTHING`.

> **No hardcoded admin password:** `init.sql` no longer seeds a user. On first boot,
> `AdminBootstrap` creates `admin` with a randomly generated password, writes the plaintext once
> to `.admin-credentials` (POSIX 600) and instructs you to delete it after first login. Existing
> deployments keep their credentials untouched.
...
The suite covers **37 tests across 8 classes** against an in-memory H2 database (no PostgreSQL required):
...
- `HardeningGapsTest` — NVD 2.0 JSON parsing, labeled feed fallback, client-IP target regions, bootstrap password strength/randomness, scheduled rotation from history
...
## 🛰️ Live Threat Intelligence & Autonomous Hardening

- **CVE feed (master prompt Layer 0):** every analysis queries the live
  [NVD 2.0 API](https://services.nvd.nist.gov/rest/json/cves/2.0) for CVEs matching the detected
  cipher (60-minute in-process cache; `NVD_API_KEY` optional). If NVD is unreachable the small
  static catalog is used and the response's `threatIntelFeed.feedBasis` says exactly that —
  live data is never faked.
- **Morphic defense on a clock (Layer 5):** a scheduler generates a NEW hardened configuration
  every 15 minutes (`MORPHIC_INTERVAL_MINUTES`) from the mean risk of recent analyses and
  persists it to `morphic_state`. Disable with `MORPHIC_ENABLED=false`.
- **Autonomous pre-hardening (Layer 2):** `POST /api/oracle/pre-harden` (or the dashboard
  button) applies every generated remediation plan for real — hardened config promoted to the
  active artifact, original archived for rollback — then HMAC-signs a mesh event for peer
  sectors and seals the operation into the ledger. Artifact-level only; nothing touches a live
  VPN device.
